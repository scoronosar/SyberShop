...
export const ProductPage = () => {
  ...
  const skuOptionGroups = useMemo(() => {
    ...
    skuList.forEach((sku: any) => {
      (sku.properties ?? []).forEach((prop: any) => {
        const propName = prop.prop_name || prop.name || t('product_page.option', 'Опция');
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        title: data.title,
        text: `${t('product_page.share_text', 'Проверьте этот товар')}: ${data.title}`,
        url: window.location.href,
      }).catch(() => {});
    } else {
      navigator.clipboard.writeText(window.location.href);
      toast.success(t('product_page.link_copied', 'Ссылка скопирована!'));
    }
  };
